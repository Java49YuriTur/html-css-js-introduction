...
const sectionMostPopularity = document.querySelector(".details-popularity");
const sectionLeastPopularity = document.querySelector(".details-least");
ulElement.innerHTML = getFullMoviesList();
...
function getFullMoviesList() {
    return moviesData.results.map(mov => {
        return createElement(mov);
    }).join('');
}
function createElement(mov) {
    return `<div class="movie-item">
                 <img class="movie-image" src="${httpPrefix}${mov.poster_path}">
                 <div class="details-title">${mov.original_title}</div>
                 <div class="details-overview">${mov.overview}</div>
            </div>`;
}
function getMostPopularity() {return createElement(getMostPopularMovie());}
function getLeastPopularity() {return createElement(getLeastPopularMovie());}
    
function getMostPopularMovie() {
    return moviesData.results.reduce((maxPopMov, curMov) => {
        return (maxPopMov.popularity > curMov.popularity) ? maxPopMov : curMov;
    });
}
function getLeastPopularMovie() {
    return moviesData.results.reduce((minPopMov, curMov) => {
        return (minPopMov.popularity > curMov.popularity) ? curMov : minPopMov;
    });
}
function show(ind) {
    sectionElements.forEach(section => section.hidden = true);
    sectionElements[ind].hidden = false;
}
