...
function validatePages(element) {
    ...
    if (value < MIN_PAGES || value > MAX_PAGES) {
        const message = value < MIN_PAGES ? `number of pages must be ${MIN_PAGES} or greater`: `number of pages must be ${MAX_PAGES} or less`;           
...
function validatePublishingDate(element) {
    ...
    if (value < MIN_YEAR || value > maxYear) {
        const message = value < MIN_YEAR ? `year must be ${MIN_YEAR} or greater`: `year must be ${maxYear} or less`;           
...
function validateDate(element) {
    ...
    if(selectedDate < minDate || selectedDate > maxDate) {
        const message = selectedDate<minDate ? `date must be ${minDate} or greater`:`date must be ${maxDate} or less`;          
        showErrorMessage(element, message, dateErrorElement) ;    
    }
}
...
function onChangePagesFrom(event) {
    ...
    if (pageTo && value >= pageTo) {
        showErrorMessage(event.target, "Page number 'from' must be less than Page number 'to'", pagesFormErrorElement);     
    } else {
        pageFrom = value;
    }
}
...
window.validatePublishingDate = validatePublishingDate;